...
let obj = {};
function maxWordInAString(str) {
  let newStr = str.split(" ").join(" ");
  console.log(newStr);
  let maxWord = "";
  for (let word of newStr) {
    obj[word].length ? (obj[word] = 0) : obj[el].length;
  }

  for (let el in obj) {
    obj[el] > maxWord ? (maxWord = obj[el]) : (maxWord = maxWord);
  }
  return maxWord;
}

console.log(maxWordInAString("Find the Largest word"));
